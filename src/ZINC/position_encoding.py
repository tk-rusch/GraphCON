...
POS_ENC_PATH = os.path.join("dataset", "pos_encodings")
...
def find_or_make_encodings(dataset, k, max_evector_dim=64):
    ...
    # - otherwise, calculate...
        ...
        _, eigenvectors = get_laplacian_evectors(A, max_evector_dim + 1)
        # - ... and store them on disk
        if not os.path.exists(POS_ENC_PATH):
            os.makedirs(POS_ENC_PATH)
        with open(fname, "wb") as f:
            pickle.dump(eigenvectors[:, 1:], f)
